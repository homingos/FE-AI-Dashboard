...
export const NameValueRow: React.FC<NameValueRowProps> = ({
  ...
  return (
    <div className="flex flex-row w-full items-baseline text-sm">
      ...
      <div className={cn(`text-xs shrink text-right`, `text-${valueColor}`)}>
        {value}
      </div>
    </div>
  );
    ...
    return (
      <div className="flex flex-row w-full items-baseline text-sm">
        ...
        <input
          ...
          className={cn(`text-xs shrink text-right bg-transparent border-b border-gray-800 focus:outline-none focus:border-gray-600 px-2 py-0`, `text-${valueColor}`)}
